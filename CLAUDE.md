...
| `trace.sh` | 代码 @req 溯源覆盖率 | 从 `requirements.trace.yaml` sidecar 读取 trackable 列表，在 src/ + tests/ 中 grep `@req <ID>`；输出逐条 ✅/❌ 和覆盖率；`--strict` 有未覆盖则 exit 1；`--sync` 将有代码覆盖的 `open` 回写为 `resolved` | 验证阶段、closeout 内部调用 |
