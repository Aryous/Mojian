...
> 由 tech-selection agent 基于 requirements.md (approved) 和 `.claude/ARCHITECTURE.md` 输出。
...
**理由**：
1. **架构最简**：纯客户端 SPA 不需要 SSR/RSC，Vite 不引入任何服务端概念，与 `.claude/ARCHITECTURE.md` 的分层模型零摩擦
...
**理由**：
1. **架构映射最直接**：Zustand 的 store 模式完美映射 `.claude/ARCHITECTURE.md` 的 Runtime 层——每个 store 文件对应 `src/runtime/store/` 下一个模块（resumeStore、editorStore、aiStore 等），store 内调用 Service 层方法，UI 通过 hooks 消费 store
...
**未选原因**：
- **Jotai**：原子化模型在处理简历这样的嵌套结构化数据时，原子之间的派生和组合关系会变得复杂。更关键的是，原子天然分散，与 `.claude/ARCHITECTURE.md` 要求的 `src/runtime/store/` 集中管理模式冲突——Jotai 的原子倾向于分散到各个组件附近，这会模糊 Runtime 层的边界
...
**背景**：AI 优化是 P0 功能，初始供应商为 OpenRouter，架构预留多供应商切换。`.claude/ARCHITECTURE.md` 要求所有 AI 调用统一经过 `src/service/ai/provider.ts`。OpenRouter API 兼容 OpenAI Chat Completions 格式。需要支持：流式响应（优化结果逐步呈现）、结构化输出（`response_format`）、错误处理（API Key 无效/余额不足/超时）。
...
- **Vercel AI SDK**：功能强大但为墨简过度设计。它的核心价值在于 `useChat`/`useCompletion` 等 React hooks 和 Next.js API Route 集成，这些都假设存在服务端。墨简是纯客户端应用，SDK 的服务端适配层完全用不上。且 hooks 默认运行在 UI 层，与 `.claude/ARCHITECTURE.md` 的分层规则冲突——需要额外工作将 AI 调用隔离到 Service 层
...
## 决策 15：AI 优化调用 JSON 输出可靠性方案

**日期**：2026-03-28

**背景**：当前 `optimizeResume` 调用（`src/service/ai/optimize.ts`）依赖 system prompt 指示 AI 返回 JSON，再由 `extractJsonFromText`（正则提取）+ Zod schema 校验解析结果。实际使用中出现 AI 返回不完整 JSON 导致 Zod 校验失败的问题。根因诊断发现 `src/config/ai.ts` 中 polish 和 concise 策略的 systemPrompt 末尾包含"输出格式与输入一致（纯文本）"的指令，而 `buildSystemMessage` 会在这些 systemPrompt 后面追加"你必须且只能输出一个合法的 JSON 对象/数组"的 JSON 格式指令。两条矛盾指令同时存在于 system prompt 中，导致模型输出不确定——有时返回纯文本解释，有时返回不完整 JSON，有时正确返回 JSON。提议在 API 层使用 `response_format: { type: "json_object" }` 参数从根本上强制 JSON 输出。

**候选方案**：

| 维度 | A: response_format: json_object | B: response_format: json_schema (Structured Outputs) | C: 仅修复 prompt 矛盾（不加 response_format） |
|---|---|---|---|
| JSON 保证强度 | 强。模型级约束，保证输出是合法 JSON（但不保证 schema 合规） | 最强。模型通过 Constrained Decoding 保证输出严格匹配 JSON Schema | 弱。完全依赖 prompt 遵从性，模型仍可能返回非 JSON |
| 模型兼容性 | 广泛。OpenRouter 上 gpt-4o-mini、gpt-4o、Claude、Gemini、大多数主流模型均支持 | 较窄。仅 gpt-4o-mini-2024-07-18+、gpt-4o-2024-08-06+ 等 OpenAI 模型支持。Claude/Gemini/开源模型通过 OpenRouter 不支持 | 无限制。任何模型均可用 |
| 多供应商适配 | 好。若未来切换模型（如 Claude、Gemini），json_object 支持度广泛 | 差。锁死在 OpenAI 系列模型上。R3.2（多 AI 供应商配置）要求架构预留供应商切换能力，json_schema 与此冲突 | 最好。无 API 层依赖 |
| 与现有 Zod 校验的关系 | 互补。json_object 保证 JSON 语法合法，Zod 保证结构合规。两层各司其职 | 重叠。json_schema 自身保证结构合规，Zod 校验变为冗余的第三层 | Zod 是唯一结构校验层 |
| 实现改动量 | 极小。在 `client.chat.completions.create()` 调用中添加一个参数。同时需修复 config/ai.ts 中的 prompt 矛盾 | 中。需将 Zod schema 转为 JSON Schema 格式传入 API，且需处理不支持该参数的模型的 fallback 逻辑 | 极小。仅修改 config/ai.ts 中 2 处 prompt 文案 |
| 风险 | 低。OpenAI 文档要求 prompt 中包含"json"一词（已满足：现有 prompt 多处提及 JSON），否则可能无限生成空白。OpenRouter 对不支持该参数的模型会忽略该参数（非致命错误），但可通过 `require_parameters` 收紧 | 低（功能层面），但高（架构层面）——锁定供应商 | 中。修复 prompt 矛盾可改善输出质量，但无法杜绝模型偶发的非 JSON 输出 |
| Agent 可读性 | 极高。`response_format: { type: "json_object" }` 是 OpenAI SDK 最常见的用法之一，AI Agent 训练集覆盖度极高 | 高。Structured Outputs 是较新特性，但文档完善 | 不涉及代码变更，无可读性考量 |

**决策**：A + C — 在 API 层添加 `response_format: { type: "json_object" }`，同时修复 config/ai.ts 中 prompt 的矛盾指令

**理由**：
1. **分层防御**：json_object（API 层保证 JSON 语法合法）+ Zod（代码层保证结构合规）+ prompt 指令（引导模型输出正确结构）三层各司其职。API 层消除了"AI 返回非 JSON"这一整类错误，Zod 继续负责"JSON 合法但结构不符"的校验，prompt 负责引导 schema 正确性。这与决策 12（Zod 校验方案）完全互补而非冲突
2. **多供应商兼容**：json_object 在 OpenRouter 上的支持度远广于 json_schema。gpt-4o-mini（当前默认模型）、gpt-4o、Claude 3.5 Sonnet、Gemini 1.5 Pro 等主流模型均支持。即使未来切换模型，json_object 的可用性远高于 json_schema，符合 R3.2（多供应商配置）的架构预留
3. **改动最小、风险最低**：仅需在 `optimize.ts` 的 `client.chat.completions.create()` 调用中添加 `response_format: { type: "json_object" }`，以及修复 config/ai.ts 中 2 处 systemPrompt 的矛盾文案。不引入新依赖、不改变现有数据流
4. **根因修复**：方案 C（修复 prompt 矛盾）是必须做的，因为矛盾指令本身就是 bug——即使有 json_object 参数，prompt 中同时说"纯文本"和"JSON"也会降低模型对 JSON schema 的遵从质量。A + C 同时消除根因和症状

**未选原因**：
- **B: response_format: json_schema (Structured Outputs)**：功能上最强（保证 schema 合规），但与 R3.2（多 AI 供应商配置）直接冲突。json_schema 的 Constrained Decoding 是 OpenAI 模型特有的能力，通过 OpenRouter 调用 Claude/Gemini/开源模型时不支持此参数。采用 json_schema 意味着要么锁死在 OpenAI 模型上，要么实现复杂的 fallback 逻辑（支持时用 json_schema，不支持时降级为 json_object + Zod）。此外，json_schema 要求将 Zod schema 转换为 JSON Schema 格式传入 API，与现有的 Zod 校验形成冗余——两层都在做结构校验，职责不清晰。在墨简当前规模下（6 种固定 section 类型），Zod 校验已足以保证结构合规，json_schema 的增量收益不值得付出供应商锁定和架构复杂度的代价
- **C: 仅修复 prompt 矛盾（不加 response_format）**：修复 prompt 矛盾是必须的，但仅靠 prompt 无法提供硬保证。即使 prompt 完全正确且无矛盾，模型仍可能在长输出时截断 JSON（max_tokens 不足）、在 JSON 前后附加解释文字、或在 edge case 下返回非 JSON。json_object 参数在模型生成层面提供了 prompt 无法替代的约束力。单独修复 prompt 是"消除根因但不加固防线"，A + C 才是完整方案

**约束条件**：
1. `response_format: { type: "json_object" }` 添加到 `src/service/ai/optimize.ts` 中 `optimizeResume` 的 API 调用。`classifyUserIntent` 不需要此参数（它返回的是单个策略 ID 文本，不是 JSON）
2. OpenAI API 要求使用 json_object 时 prompt 中必须包含"json"一词——当前的 `buildSystemMessage` 已在输出格式指令中多处使用"JSON"，满足此前提条件
3. config/ai.ts 中 polish 和 concise 策略的 systemPrompt 中"输出格式与输入一致（纯文本）"需修改为与 JSON 输出指令一致的表述（如删除该行，因为 JSON 输出格式已由 `buildSystemMessage` 统一追加）
4. OpenRouter 的 Response Healing 插件可作为额外安全网启用（修复 JSON 语法错误如尾逗号、未闭合括号），但这是运行时配置项，不在代码层面决策
5. 如果未来切换到不支持 json_object 的模型，现有 `extractJsonFromText`（正则提取）+ Zod 校验的 fallback 链路仍然可用——只需移除 response_format 参数即可降级

---

## 决策 16：API Key 存储安全（F19）

**日期**：2026-03-28

**背景**：走查发现 F19（约束遗漏）指出当前 API Key 以明文存储在 localStorage（键名 `mojian:openrouter-api-key`，定义在 `src/config/ai.ts`，读写在 `src/repo/settings.ts`）。localStorage 的特性是同域任何 JavaScript 均可读取，一旦发生 XSS 攻击，API Key 直接泄露。墨简是纯前端 SPA，无后端代理，因此不存在"API Key 只在服务端使用"这一根本性安全方案。所有浏览器端存储方案对 XSS 攻击都是透明的，真正的防线是 CSP 策略和 XSS 预防，而非存储位置。当前项目尚无 CSP 配置。决策需要在安全性和用户体验之间取得平衡。

**候选方案**：

| 维度 | A: localStorage + 简单混淆 (btoa/atob) | B: sessionStorage（关闭即清） | C: sessionStorage + 简单混淆 | D: 仅内存存储（Zustand store，刷新即丢） |
|---|---|---|---|---|
| XSS 防护增量 | 无。btoa 是可逆编码（非加密），XSS 读到编码值后 atob 即可还原。对自动化脚本的阻挡力为零 | 无。sessionStorage 与 localStorage 一样，同域 JS 完全可读 | 无。同 A + B 的叠加，编码不增加安全性 | 略高。Key 不在持久化存储中，Chrome DevTools Storage 面板无法直接看到。但运行时 JS 仍可通过 Zustand store 的 getState() 读取 |
| 被动泄露面 | 高。Key 持久化存储，即使用户关闭浏览器后重新打开，恶意扩展或第三方脚本仍可读取 | 中。关闭标签页后 Key 消失，攻击窗口限于当前会话 | 中。同 B | 低。刷新页面后 Key 消失，攻击窗口最短 |
| 用户体验 | 最佳。配置一次，永久可用。跨标签页/刷新/重启浏览器均保持 | 中。同一标签页内保持，关闭标签页后需重新输入。跨标签页不共享 | 中。同 B | 最差。每次刷新页面都要重新输入 Key。极高摩擦 |
| 架构改动量 | 极小。在 `src/repo/settings.ts` 的 get/set 中加 btoa/atob | 小。将 localStorage 改为 sessionStorage | 小。sessionStorage + btoa/atob | 中。需将 Key 从 Repo 层（持久化）移至 Runtime 层（Zustand store），同时修改 `src/service/ai/provider.ts` 的读取路径，以及所有 UI 中读取 Key 的地方 |
| 架构合规 | 合规。Key 读写仍在 Repo 层 | 合规。sessionStorage 仍是持久化存储的一种，属于 Repo 层 | 合规。同 B | 有摩擦。API Key 从 Repo 层（持久化数据）移到 Runtime 层（内存状态）。ARCHITECTURE.md 将 Repo 定义为"用户数据"层，将 Key 移出 Repo 模糊了持久化边界 |
| Agent 可读性 | 极高。localStorage API 最基础 | 极高。sessionStorage API 与 localStorage 几乎相同 | 高。多一层编码 | 中。需要理解 Zustand store 中的敏感数据管理模式 |

**决策**：B — 改用 sessionStorage

**理由**：
1. **缩小被动泄露面是唯一有效的增量改善**：在纯前端应用中，任何 JS 可访问的存储对主动 XSS 攻击都是透明的。但被动泄露面有差异——localStorage 中的 Key 在用户关闭浏览器后仍然存在，恶意浏览器扩展或用户离开电脑后他人可从 DevTools 查看。sessionStorage 将 Key 的生命周期限制在当前标签页会话，关闭标签页即清除，显著缩小了攻击窗口
2. **用户体验可接受**：用户在同一标签页内（包括页面内导航）Key 保持有效，仅在关闭标签页或打开新标签页时需重新输入。对于简历编辑这种"单次长会话"的使用模式，影响有限。且 sessionStorage 在同一标签页内的页面刷新（F5）也保持数据——这与纯内存方案（D）有本质区别
3. **改动最小**：仅需将 `src/repo/settings.ts` 中 3 处 `localStorage` 调用改为 `sessionStorage`，零新依赖、零架构变更、零类型变更。`src/config/ai.ts` 的键名定义和 `src/service/ai/provider.ts` 的读取逻辑完全不变
4. **与真正的安全防线互补**：sessionStorage 不是防 XSS 的手段（不应有此幻想），但它与后续应补齐的 CSP 策略形成分层防御——CSP 阻止恶意脚本注入（防主动攻击），sessionStorage 缩小 Key 的暴露时间窗口（降低被动泄露风险）

**未选原因**：
- **A: localStorage + 简单混淆 (btoa/atob)**：btoa 是 Base64 编码，不是加密。`atob(localStorage.getItem('mojian:openrouter-api-key'))` 与 `localStorage.getItem('mojian:openrouter-api-key')` 对攻击者而言只差一次函数调用。混淆给出了虚假的安全感，同时保留了 localStorage 的最大泄露面（Key 永久存在）。在安全工程中，混淆（obfuscation）从不被视为安全层，将其作为改善措施是误导性的。唯一比不改更差的方案——因为它让人误以为问题已解决
- **C: sessionStorage + 简单混淆**：同 A 对混淆的批评。sessionStorage 本身已足够，加 btoa/atob 不增加任何实际安全性，只增加代码复杂度。混淆的唯一合理理由是"防止开发者在 DevTools 中意外看到明文"，但这不是安全需求，且 sessionStorage 在标签页关闭后自动清除，DevTools 暴露窗口已被限制
- **D: 仅内存存储（Zustand store）**：安全性最高（Key 从不接触任何浏览器存储 API），但用户体验代价过大——每次刷新页面都需要重新粘贴 API Key。简历编辑过程中误触 F5 或浏览器更新导致页面刷新是常见场景，每次都丢失 Key 会严重打断工作流。此外，将 Key 从 Repo 层移到 Runtime 层与 ARCHITECTURE.md 的分层模型产生摩擦——API Key 在语义上是"用户配置数据"（Repo/Config 层），不是"应用运行时状态"（Runtime 层）。最后，sessionStorage 已经在刷新时保持数据的同时提供了"关闭即清"的安全特性，D 方案的增量安全收益（仅防刷新后的 DevTools 查看）不值得付出刷新丢失的 UX 代价

**约束条件**：
1. 本决策以"纯前端 SPA，无后端代理"为前提。若未来引入 BFF（Backend for Frontend），应重新评估将 AI 调用代理到服务端的方案——那时 API Key 可存储在服务端环境变量中，根本不暴露给浏览器
2. sessionStorage 不跨标签页共享。如果用户在多个标签页打开墨简，每个标签页需要独立配置 Key。这是 sessionStorage 的固有特性，目前可接受（简历编辑器的多标签页场景少见）
3. 本决策不替代 CSP 策略。应在后续阶段补齐 Content-Security-Policy 响应头（或 meta 标签），作为 XSS 防御的真正主线。CSP 配置属于部署层决策，不在本次技术选型范围内，但此处记录为待办
...
**裁决**：A — 保持 Zod 3（`^3.24.0`），已在用且稳定
**裁决人**：Lucas
**日期**：2026-03-28
**已回注**：frontmatter `approved_by/open_questions` 已更新；当前依赖继续保持 `zod` 3.x
...
| Q2 | Zod 版本选择 | A: 保持 Zod 3（^3.24.0），已在用且稳定 | Lucas | 2026-03-28 |

## 溯源表

| 输入条目 | 处理 | 输出位置 | 备注 |
|---|---|---|---|
| R1.2 简历持久化 | 已覆盖 | 决策 3 | IndexedDB / Dexie 方案 |
| R2.1 简历渲染 | 已覆盖 | 决策 4 | Typst WASM 渲染 |
| R2.3 简历导出 | 已覆盖 | 决策 5 | PDF 导出链路 |
| R3.1 AI 优化选项 | 已覆盖 | 决策 6, 决策 15 | AI 调用层与 JSON 输出可靠性 |
| R3.2 多 AI 供应商配置 | 已覆盖 | 决策 6, 决策 15 | OpenRouter + OpenAI SDK + json_object 兼容性 |
| R3.3 AI 上下文工程 | 已覆盖 | 决策 12, 决策 13, 决策 14, 决策 15 | schema / id / diff / JSON 可靠性 |
| F19 API Key 存储安全 | 已覆盖 | 决策 16 | sessionStorage 方案 + CSP 待办 |
| R4.1 古风视觉体系 | 显式排除 | — | 视觉规范由 design-spec.md 覆盖 |
