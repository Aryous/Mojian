...
### Q1: 宣纸纹理实现方案
**背景**：宣纸纹理是全局视觉基础，有两种实现路径，影响性能和视觉质量的平衡。
**选项**：
- A: 纯 CSS 方案（SVG 内联噪点 + 多层渐变），零网络请求，但视觉精度有限
- B: 微型纹理图片（≤5KB base64 内联），视觉更真实，但增加 CSS 体积
**影响**：纹理方案确定后，所有使用宣纸背景的组件（PaperCard、页面背景等）统一遵循
**阻塞**：design-phase-b（组件实现）

### Q2: 窗棂图案选择
**背景**：窗棂有多种传统几何图案，需选定一种作为墨简的标志性图案。
**选项**：
- A: 回纹（雷纹）——最简洁的连续几何图案，线条感强，适合细边框
- B: 万字纹——更具传统辨识度，但图案复杂度更高
- C: 冰裂纹——自然不规则，但可能与墨线分割线风格冲突
**影响**：窗棂图案将用于工具栏装饰线、面板边框、对话框装饰，是品牌视觉识别的一部分
**阻塞**：design-phase-b（LatticePattern 组件实现）

### Q3: 墨砚 AI 面板的交互位置
**背景**：AI 优化面板需要在编辑时随时可达，但不能遮挡编辑内容。
**选项**：
- A: 右侧抽屉——从预览区右侧滑出，保持编辑区不受影响
- B: 底部面板——从底部升起，类似 IDE 的终端面板
- C: 内联面板——在编辑区选中文本后，在选中位置附近弹出浮动面板
**影响**：AI 面板位置决定了编辑-优化-确认的核心工作流，是用户体验的关键路径
**阻塞**：design-phase-b（AIOptionPanel 布局）、feature agent（AI 交互实现）
